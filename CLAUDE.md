...
## Account Deletion (GDPR — shipped 2026-06)
Canonical spec: [`../DatingAppProj/ACCOUNT_DELETION_IOS_SPEC.md`](../DatingAppProj/ACCOUNT_DELETION_IOS_SPEC.md) (As-Built section). `DELETE /users/{user_id}` ([`routers/users.py`](app/routers/users.py) `delete_account`) is a **soft-delete + full PII scrub** (GDPR Art. 17): stamps `deleted_at`, sets `is_active=False`, replaces `firebase_uid`/`email`/`phone_number` with synthetic values, nulls every other PII column **including `birthdate`**, and **deletes the user's `age_attestations` + `birthdate_change_requests` rows** (they hold DOB). Keeps only `id`, timestamps, and `strikes` (moderation audit). Deactivates all of the user's conversations via `deactivate_user_conversations`. Idempotent (the `deleted_at IS NULL` filter → 404 on a second call); 403 unless the token UID matches. Rate-limited 3/hour (IP-keyed). The Firebase Auth user is deleted **client-side** after this succeeds (backend is the source of truth). Churn reasons: anonymous append-only `deletion_reasons` table (no user id) via `POST /deletion-reasons` ([`routers/deletion_reasons.py`](app/routers/deletion_reasons.py), 204, 1/min) — DB CHECK + Pydantic both enforce `other`-requires-free-text. Migration: `alembic/versions/c4d5e6del01_add_deletion_reasons_table.py`. Tests: `tests/test_users_delete.py`, `tests/test_deletion_reasons.py`.
